...
public class Sym0014Service {
	 ...
	 * @param model
	 */
	public void initData(Model model, String countryId , String cityId, String districtId) {
		...
		try {
			...
			countryData = Sym0014Dao.getQltnMCountryMapper().selectByExample(qltnMCountryExample);
		...
		try {
			...
			cityData = Sym0014Dao.getQltnMCityMapper().selectByExample(qltnMCityExample);
		...
		try {
			...
			cityCriteria.andDeleteFlagEqualTo(Constants.DELETE_FLAG_OFF);
			districtData = Sym0014Dao.getQltnMDistrictMapper().selectByExample(QltnMDistrictExample);
     ...
     */
	public List<Sym0014Result> searchData(Sym0014Conditions searchConditions) {
		List<Sym0014Result> bnn0014ResultList = new ArrayList<Sym0014Result>();
        HashMap<String, Object> params = createSearchConditionParams(searchConditions);
        try {
            logger.info(LoggerMessage.SEARCHING_STRATED);
            // search starts
            bnn0014ResultList = Sym0014Dao.getSym0014Mapper().searchData(params);
            logger.info(LoggerMessage.EXECUTION_FINISHED);
            if (bnn0014ResultList.size() > 0) {
                // "real" total from search result
                String searchDataTotalCounts = Sym0014Dao.getSym0014Mapper().getSearchDataTotalCounts(params);
            ...
            ex.printStackTrace();
            Sym0014Result tempObj = new Sym0014Result();
     ...
     */
    private HashMap<String, Object> createSearchConditionParams(Sym0014Conditions searchConditions) {
        ...
        // wards Name
        params.put("wardsName", searchConditions.getWardsName().equals("") ? "" : "%" + searchConditions.getWardsName() + "%");
        // wards Code
        params.put("wardsCode", searchConditions.getWardsCode().equals("") ? "" : "%" + searchConditions.getWardsCode() + "%");
        // From parameter
     ...
     */
    private void convertSanitize(List<Sym0014Result> inputData) {
        for (int i = 0; i < inputData.size(); i++) {
        	Sym0014Result currentData = inputData.get(i);
            // Wards Name
            currentData.setWardsName(Util.convertSanitize(currentData.getWardsName()));
            // Wards Code
            currentData.setWardsCode(Util.convertSanitize(currentData.getWardsCode()));
        }
    }
     ...
     * 
     * @param wardsData : data received from client
     * @return String : update successfully: 1/update failed: -1
     */
    public String updateData(QltnMWards wardsData) {
        // variable definition
        ...
        try {
            ...
            if (!checkInputBlankFields(wardsData)) {
            	// blank field(s)
            ...
            try {
            	QltnMWards wardsObj = new QltnMWards();
            	// Wards Id
            	wardsObj.setWardsId(wardsData.getWardsId());
                // wards Name
            	wardsObj.setWardsName(wardsData.getWardsName());
            	// wards Code
            	wardsObj.setWardsCode(wardsData.getWardsCode());
            	// update User id
        		wardsObj.setUpdateUserId(Util.getUserInfo().getID());
                // delete flag
                wardsObj.setDeleteFlag(wardsData.getDeleteFlag());

                int result = Sym0014Dao.getQltnMWardsMapper().updateByPrimaryKeySelective(wardsObj);
     ...
     */
    public String insertData(QltnMWards wardsData) {
        // variable definition
        ...
        try {
            ...
            try {
            	HashMap<String, Object> params = new HashMap<String, Object>();
            	params.put("wardsIdDefault", Constants.DEFAULT_PARAMS);
            	String idNumberStr = Sym0014Dao.getSym0014Mapper().getLastIdWards(params);
            	int idNumber = 0;
            	if (idNumberStr != null) {
            	...
            	if (idNumber < Constants.DEFAULT_ID) {
            		idNumber = idNumber + 1;
            		String wardsID = Constants.WARDS_CHARATER + String.format("%0" + Constants.MAX_LENGHT_ID + "d", idNumber);
            		QltnMWards wardsObj = new QltnMWards();
            		// Country Id
            		wardsObj.setCountryId(wardsData.getCountryId());
            		// city Id
            		wardsObj.setCityId(wardsData.getCityId());
            		// district Id
            		wardsObj.setDistrictId(wardsData.getDistrictId());
            		// wards Id 
            		wardsObj.setWardsId(wardsID);
            		// wards Code
            		wardsObj.setWardsCode(wardsData.getWardsCode());
                    // wards Name
            		wardsObj.setWardsName(wardsData.getWardsName());
                    ...
                    int result = Sym0014Dao.getQltnMWardsMapper().insert(wardsObj);
    ...
    /**
     * Delete Wards from DB
     * 
     * @param WardsID : wards's id to delete
     * @return String : delete successfully: 1/delete failed: -1
     */
    public String deleteData(String wardsId) {
    	// variable definition
        ...
        try {
            ...
            TransactionStatus status = txManager.getTransaction(def);
            try {
            	QltnMWards QltnMWards = new QltnMWards();
            	QltnMWards.setWardsId(wardsId);
            	QltnMWards.setDeleteFlag(Constants.DELETE_FLAG_ON);
            	// update city id
            	QltnMWards.setUpdateUserId(Util.getUserInfo().getID());
                int result = Sym0014Dao.getQltnMWardsMapper().updateByPrimaryKeySelective(QltnMWards);
    ...
    /**
     * Get wards information based on wards's id
     * 
     * @param districtID : wards's id received from client
     * @return city data
     */
	public QltnMWards getSingleData(String districtId) {
		return Sym0014Dao.getQltnMWardsMapper().selectByPrimaryKey(districtId);
	}
     ...
     */
 	private boolean checkInputBlankFields(QltnMWards wardsData) {
 		if (wardsData.getWardsName().equals("") || wardsData.getWardsCode().equals("")) {
 			return false;
 		} else {
 			return true;
 		}
