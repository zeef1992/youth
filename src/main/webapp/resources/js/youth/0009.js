...
$(document).ready(function(){
	...
	function setPopupControlState(mode) {
		...
		if (mode == MODE_NEW) {
			...
			// cates id
			$("#txtCateIdPopup").prop("disabled", false);
			// cates name
			$("#txtCateNamePopup").prop("disabled", false);
			// status
	...
	$("#btnRegisterPopup").bind("click", function() {
		...
		if (currentMode == MODE_NEW) {
			...
			if (!checkInputBlankFields()) {
				...
				// get cate input data
				dataObject = createcateDataObject();
					...
					success: function(returnedJsonData) {
						...
						if (returnedJsonData != "") {
							...
							if (returnedJsonData == VALIDATE_BLANK_FIELDS) {
								...
								$.ajax({
									url: "getSingleData",
									data: { "cateId": $("#txtCateIdPopup").val() },
									type: "POST",
									async: false,
									success: function(returnedJsonData) {
										if (checkSessionTimeout(returnedJsonData) == 1) return;
	...
	function createcateDataObject() {
		return {
			cateId: $("#txtCateIdPopup").val(),
			cateName: $("#txtCateNamePopup").val(),
