...
$(document).ready(function(){
							...
							if (returnedJsonData == VALIDATE_BLANK_FIELDS) {
								...
								$.ajax({
									...
									success: function(returnedJsonData) {
										...
										if (returnedJsonData != "") {
											// District Code
											$("#row" + selectedRowIndex).find("td").eq(0).text(returnedJsonData.wardsCode);
											// District name
											$("#row" + selectedRowIndex).find("td").eq(1).text(returnedJsonData.wardsName);
										}
									},
				...
				if (returnedJsonData != "") {
					...
					if (parseInt(totalResultCount) == -1) {
						...
						for (var i = 0; i < returnedJsonData.length; i++) {
							...
							// district Code
							tableStringArray.push("<td class='align-center goTo0015' id= '"+returnedJsonData[i].cityId+"' name = '"+returnedJsonData[i].countryId+"' districtId = '"+returnedJsonData[i].districtId+"' wardsId = '"+returnedJsonData[i].wardsId+"'>" + returnedJsonData[i].wardsCode + "</td>");
							// district name
	...
	$(document).on("click", ".delete", function() {
		...
		if (selection) {
			...
			wardsIdPopup = $("#row" + selectedRowIndex).find("td").eq(0).attr("wardsId");
	...
	$(document).on("click", ".edit", function() {
		...
		selectedRowIndex = $(this).attr("name");
		// get id of selected cate
		districtIdPopup = $("#row" + selectedRowIndex).find("td").eq(0).attr("wardsId");
		// make Ajax call to server to get data
		$.ajax({
			url: "getSingleData",
	...
	function initComboboxData(isPopupMode) {
		...
		if (districtData != "") {
			...
			for (var i = 0; i < districtData.length; i++) {
				if (districtIdDefault != "") {
					if (districtIdDefault == districtData[i].districtId) {
						optionStr += "<option value='" + districtData[i].districtId + "' selected = 'selected'>" + districtData[i].districtName + "</option>";
						$('#textDistrictName').text(districtData[i].districtName);
					} else {
						optionStr += "<option value='" + districtData[i].districtId + "'>" + districtData[i].districtName + "</option>";
					}
				} else {
					if (i == 0) {
						optionStr += "<option value='" + districtData[i].districtId + "' selected = 'selected'>" + districtData[i].districtName + "</option>";
