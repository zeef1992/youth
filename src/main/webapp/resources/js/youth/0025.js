...
$(document).ready(function(){
	...
	function setPopupControlState(mode) {
		if (mode == MODE_NEW) {
			...
			$("#txtCriteriaIdPopup").prop("disabled", true);
			// cates name
			$("#txtCriteriaNamePopup").prop("disabled", true);
			// status
	...
	$(document).on("click", ".delete", function() {
		...
		jQuestion_warning(DELETE_CONFIRM_MESSAGE, DIALOG_TITLE, DIALOG_YES_BUTTON, DIALOG_NO_BUTTON, function(val) {
			...
			criteriaIdPopup = $("#row" + selectedRowIndex).find("td").eq(1).text();
		...
		$.ajax({
			...
			success: function(returnedJsonData) {
				...
				if (returnedJsonData != "") {
					...
					// report name
					$("#txtReportNamePopup").val($("#row" + selectedRowIndex).find("td").eq(3).text());
					// detail report id
					$("#txtDetailReportIdPopup").val(returnedJsonData.detailReportId);
					// detail report name
					$("#txtDetailReportNamePopup").val($("#row" + selectedRowIndex).find("td").eq(4).text());
					// criteria id
